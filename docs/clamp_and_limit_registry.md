...
| **CL-04** | Spatial Bridge Displacement (`alg4`) | `aperture.py:115` | Asymmetric Order-4 Algebraic Limiter (`alg4`) | $[-9.11\text{ dB}, +9.11\text{ dB}]$ | ✅ **Evaluated & Verified** ($< 0.10\text{ dB}$ error for $\|\Delta G\| \le 6\text{ dB}$) |
...
| **CL-12** | Frontend Ultrasonic Rolloff | `staging.py:330-336` | `hf_excess = (1/1.2) * logaddexp(0, 1.2*(clamped - 1.5))` | Max $20\text{ kHz} = +1.49\text{ dB}$ | ✅ **Evaluated & Verified** (Guardrail 5.3.6: $< +2.0\text{ dB}$ at $20\text{ kHz}$) |
...
4. **Spatial Bridge Excursion Limiter (`alg4`):**
   Evaluated across all 735 pairs: Mudbucker neck ($+9.11\text{ dB}$) and 60s J bridge ($-9.11\text{ dB}$) retain full excursion while sub-audible DC gain remains strictly within $[-5.85\text{ dB}, +7.10\text{ dB}] \subset [-12\text{ dB}, +12\text{ dB}]$.
...
8. **Frontend Ultrasonic Rolloff:**
   Verified across all instruments: $20\text{ kHz}$ ultrasonic gain is strictly between $-16.71\text{ dB}$ and $+1.49\text{ dB} < +2.0\text{ dB}$.
